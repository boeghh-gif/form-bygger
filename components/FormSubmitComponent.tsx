...
function FormSubmitComponent({ formUrl, content }: { content: FormElementInstance[]; formUrl: string }) {
  ...
  const submitForm = async () => {
    ...
    if (!validForm) {
      ...
      toast({
        title: "Error",
        description: "Formularen er ikke udfyldt korrekt",
        variant: "destructive",
      });
      return;
    }
    ...
    return (
      <div className="flex justify-center w-full h-full items-center p-8">
        <div className="max-w-[620px] flex flex-col gap-4 flex-grow bg-background w-full p-8 overflow-y-auto border shadow-xl shadow-blue-700 rounded">
          <h1 className="text-2xl font-bold">Formular indsendt</h1>
          <p className="text-muted-foreground">Tak fordi du indsendte formularen, du kan lukke denne side nu.</p>
        </div>
      </div>
    );
  }
